...
public class CompatClientHelper {
	public static EnumActionResult processRightClick(Minecraft mc, EntityPlayerSP thePlayer, BlockPos blockpos, EnumHand enumhand, ItemStack itemstack) {
		return mc.playerController.processRightClickBlock(thePlayer, mc.world, itemstack, blockpos, mc.objectMouseOver.sideHit, mc.objectMouseOver.hitVec, enumhand);
	}


	public static IVertexBuffer wrap(VertexBuffer buffer) {
		return new Wrapper(buffer);
	}

	public static VertexBuffer unwrap(IVertexBuffer buffer) {
	...
	public static class Wrapper implements IVertexBuffer {
		...
		public Wrapper(VertexBuffer input) {
			this.input = input;
		}
		...
		public VertexBuffer.State getVertexState() {
			return input.getVertexState();
		}

		public void setVertexState(VertexBuffer.State state) {
			input.setVertexState(state);
		}
		...
		private IVertexBuffer checkWrap(VertexBuffer lightmap) {
