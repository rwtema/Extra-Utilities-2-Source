...
public abstract class XUTConTextureBase extends AbstractColoredTexture {
	...
	protected XUTConTextureBase(TextureAtlasSprite baseTexture, String spriteName) {
...
//			logImg(curValues, width, height);
	...
	protected void processData(int[][] data) {
		...
		preProcess(data[0], valueOverrides, colorOverrides);
		getLimits(data[0]);
	...
	protected int colorPixel(int pixel, int mipmap, int pxCoord) {
		Integer valueOverride = getMipmapValue(valueOverrides, pxCoord, mipmap);
		...
		Integer colorOverride = getMipmapValue(colorOverrides, pxCoord, mipmap);
